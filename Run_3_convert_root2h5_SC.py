...
def crop_jet(imgECAL, iphi, ieta, jet_shape=125):
    ...
    off = jet_shape//2
    #Don't need this if using supercluster
    ...
    if iphi < off:
        ...
        if (ieta-off >= 0):
            ...
            img1 = np.concatenate((imgECAL[:,ieta-off:,-diff:],imgECAL[:,0:ieta+off+1,-diff:]), axis=1)
            img2 = np.concatenate((imgECAL[:,ieta-off:,:iphi+off+1],imgECAL[:,0:ieta+off+1,:iphi+off+1]), axis=1)
            img_crop = np.concatenate((img1,img2), axis=-1)
    ...
    # Wrap-around on right side
        ...
        if (ieta-off >= 0):
            ...
            img1 = np.concatenate((imgECAL[:,ieta-off:,iphi-off:],imgECAL[:,0:ieta+off+1,iphi-off:]), axis=1)
            img2 = np.concatenate((imgECAL[:,ieta-off:,:diff+1],imgECAL[:,0:ieta+off+1,:diff+1]), axis=1)
            img_crop = np.concatenate((img1,img2), axis=-1)
    ...
    # Nominal case
        ...
        if (ieta - off >= 0):
            ...
            img_crop = np.concatenate((imgECAL[:,ieta-off:,iphi-off:iphi+off+1],imgECAL[:,0:ieta+off+1,iphi-off:iphi+off+1]), axis=1)
...
with h5py.File(f'{outStr}', 'w') as proper_data:
        ...
        for iEvt in range(iEvtStart,iEvtEnd):
            ...
            #X_CMSII            = np.stack([ECAL_energy, HBHE_energy], axis=0) # (2, 280, 360)
            #X_CMSII            = np.stack([TracksAtECAL_pt, TracksAtECAL_dZSig, TracksAtECAL_d0Sig, ECAL_energy, HBHE_energy], axis=0)
